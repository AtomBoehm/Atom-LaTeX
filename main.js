...
export default {
  activate (serialized) {
    ...
    editorActv = atom.workspace.getActiveTextEditor()
    if (editorActv.getGrammar().name == 'LaTeX') {
        latex.status.setActive()
    } else {
        latex.status.setInactive()
    }
    ...
    this.disposables.add(atom.workspace.observeActivePaneItem(item => {
        // Experimental: hide LaTeX status bar if active grammar is not LaTeX
        if (atom.workspace.isTextEditor(item)) {
            if (item.getGrammar().name == 'LaTeX') {
                latex.status.setActive()
            } else {
                latex.status.setInactive()
            }
